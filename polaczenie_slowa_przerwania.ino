#include "Talkie.h"
#include "DHT.h"
#include "PinChangeInterrupt.h" //do przerwan
#define DHT11_PIN 2
#define INTERRUPT_PIN_T 10
...
Adafruit_BMP085 bmp;
/*zmienne globalne bo do przerwan potrzebne*/
float RS_gas = 0;
float ratio = 0;
float sensorValue = 0;
float sensor_volt = 0;
float R0 = 50000.0;
int temperature_p = 0, humidity_p = 0, pressure_p = 0, ppm_p = 0;
...
/* Say any number between -9,999 and 9,999 */
...
void interruptRoutine_T() {
  digitalWrite(7, !digitalRead(7));//usunac
  Serial.println("Mowie temperature");//usunac
  Serial.println(temperature_p);
  sayNumber(temperature_p);
}
void interruptRoutine_H() {
  digitalWrite(7, !digitalRead(7));//usunac
  Serial.println("Mowie wilgotnosc");//usunac
  Serial.println(humidity_p);
  sayNumber(humidity_p);
}
void interruptRoutine_P() {
  digitalWrite(7, !digitalRead(7));//usunac
  Serial.println("Mowie cisnienie");//usunac
  Serial.println(pressure_p);
  sayNumber(pressure_p);
}
void interruptRoutine_C() {
  digitalWrite(7, !digitalRead(7));//usunac
  Serial.println("Mowie ppm");//usunac
  Serial.println(ppm_p);
  sayNumber(ppm_p);
}
...
void setup() 
{
  ...
  dht.setup(DHT11_PIN);
  bmp.begin();
  pinMode(INTERRUPT_PIN_T, INPUT_PULLUP);
  pinMode(INTERRUPT_PIN_H, INPUT_PULLUP);
  pinMode(INTERRUPT_PIN_P, INPUT_PULLUP);
  pinMode(INTERRUPT_PIN_C, INPUT_PULLUP);
  pinMode(7, OUTPUT);//usunac lampka dla testow
  attachPinChangeInterrupt(digitalPinToPinChangeInterrupt(INTERRUPT_PIN_T), interruptRoutine_T, FALLING);
...
void loop() 
{
  ...
  humidity_p = wilgotnosc;
  //Pobranie informacji o temperaturze
  //int temperatura = dht.getTemperature();
  float temperatura2 = bmp.readTemperature();
  temperature_p = (int)temperatura2;//zmienic nazwy potrzebne do przerwan
  float cisnienie = bmp.readPressure()/100;
  pressure_p = (int)cisnienie;
  //int wysokosc = bmp.readAltitude(101400);
  //czad
  ...
  /*if (dht.getStatusString() == "OK"){
    sayNumber(wilgotnosc);
    delay(3000);
    sayNumber(temperatura2);
  }*/
    delay(3000);
}
