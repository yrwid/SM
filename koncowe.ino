#include <stdlib.h>
#include "Talkie.h"             //glosnik
#include "PinChangeInterrupt.h" //przerwania
#include "DHT.h"
#include <SoftwareSerial.h>   //esp8266 - komunikacja pomiedzy esp a atmega
#define INTERRUPT_PIN_T 10    //przyciski
#define INTERRUPT_PIN_H 11
#define INTERRUPT_PIN_P 12
#define INTERRUPT_PIN_C 13
#define DHT11_PIN 6           //pin czujnika DHT
SoftwareSerial monitor(2, 4); //rx, tx pins - esp jako klient atmegi
DHT dht;                      //inicjalizacja
String GET;                   //zmienna do get request (http protocol)
#include <Wire.h>             //do czujnika bmp180 - tylko do komunikacji I2C
#include <Adafruit_BMP085.h>  //do bmp180 - odczytywanie wartosci
Adafruit_BMP085 bmp;

//String NazwaSieci = "FunBox2-8424"; //Nazwa sieci do ktorej sie laczymy
//String HasloSieci = "7FF3CCAEA329263F3C747ED73A"; //haslo do sieci
String NazwaSieci = "AndroidAP8607"; //Tu podaj nazwe swojej sieci.
String HasloSieci = "eaed34aec159"; //Tu hasło do sieci.
/*---------- zmienne do czujnika MQ-7 -----------*/
 float RS_gas = 0;
 float ratio = 0;
 float sensorValue = 0;
 float sensor_volt = 0;
 float R0 = 7000.0;   //wartosc rezystancji odczytana po "kalibracji"
/*---------- Do wywolywania wysylu dancyh z odpowidnim interwalem - millis() ----------*/
unsigned long aktualnyCzas = 0;   
unsigned long zapamietanyCzas_D = 0;//czas - dht11
unsigned long zapamietanyCzas_B = 0;//czas - bmp
unsigned long zapamietanyCzas_M = 0;//czas - mq7
int temperatura = 0;
int wilgotnosc = 0;
float temperature_p = 0, humidity_p = 0, pressure_p = 0;
String wilgotnosc_str, temperatura_str,temperatura2_str, cisnienie_str, ppm_str;

/* talkie obiekt - inicjalizacja */

Talkie voice(true, false); //true - korzystamy z pinu 3(pwm), false - nie korzystamy z pinu 11 (odwrócony pwm)
boolean tell_T = false, tell_H = false, tell_C = false, tell_P = false; //do funkcji przerwan

/* inicjalizacja wymaiwanych słów */
...
void interruptRoutine_T() {
  digitalWrite(8, HIGH);  //Zapalamy diode
  tell_T = true;          //ustawienie flagi
}
void interruptRoutine_H() {
  digitalWrite(8, HIGH);
  tell_H = true;
}
void interruptRoutine_P() {
  digitalWrite(8, HIGH);
  tell_P = true;
}
void interruptRoutine_C() {
  digitalWrite(8, HIGH);
  tell_C = true;
}
...
void setup()
{
   //Serial.begin(115200);      //USUNAC
    monitor.begin(115200);          //komunikacja esp - arduino bound rate
    monitor.println("AT+CIPMUX=1"); //ustawienie trybu wielu polaczen
    delay(200);
    connect_c();                    //polaczenie z dana siecia wifi
    dht.setup(DHT11_PIN);           //iniciajliacja dht
    bmp.begin();                    //inicjalizacja bmp
  pinMode(INTERRUPT_PIN_T, INPUT_PULLUP); //inicjalizacja przyciskow
  pinMode(INTERRUPT_PIN_H, INPUT_PULLUP);
  pinMode(INTERRUPT_PIN_P, INPUT_PULLUP);
  pinMode(INTERRUPT_PIN_C, INPUT_PULLUP);
  pinMode(8, OUTPUT);               //inicjalizacja diody
  attachPinChangeInterrupt(digitalPinToPinChangeInterrupt(INTERRUPT_PIN_T), interruptRoutine_T, FALLING); //inicjalizacja przerwan
  attachPinChangeInterrupt(digitalPinToPinChangeInterrupt(INTERRUPT_PIN_H), interruptRoutine_H, FALLING);
  attachPinChangeInterrupt(digitalPinToPinChangeInterrupt(INTERRUPT_PIN_P), interruptRoutine_P, FALLING);
  attachPinChangeInterrupt(digitalPinToPinChangeInterrupt(INTERRUPT_PIN_C), interruptRoutine_C, FALLING);


}

void loop(){
   aktualnyCzas = millis(); //sczytujemy aktualny czas
                               // if ((int)dht.getHumidity()!= 0){humidity_p = (int)dht.getHumidity();Serial.println("Jest ok");}//usunac
                                // Serial.print("wilgotnosc: "); //usunac
                                // Serial.println(humidity_p);   //usunac
   /* ----------- Odczytanie wartosci z czujnikow ------------- */
  if((int)dht.getHumidity()!=0)           //czujnik dht jak sczytuje bledne wartosci wystawia 0 - zastosowano bo jego zakres to 5-95% wiec bezpieczne
    humidity_p = dht.getHumidity();       //odczytanie wilgotnosci
  if((int)dht.getTemperature()!=0)           //czujnik dht jak sczytuje bledne wartosci wystawia 0
    temperatura_str =(String)dht.getTemperature();       //odczytanie tempeartury - tylko do wysylu
  temperature_p = bmp.readTemperature();  //odczytanie temperatury
  pressure_p = (bmp.readPressure()/100);  //odczytanie temperatury + przeskalowanie na hPa
  sensorValue = analogRead(A0);           //odczytanie wartosci z mq-7 - wartosc analogowa
  sensor_volt = sensorValue/1024*5.0;     //przeskalowanie na odpowiednie napiecie
  RS_gas = (5.0-sensor_volt)/sensor_volt; //odpowiednia proporcja
  ratio = RS_gas/R0;                      //na podstawie R0 z kalibracji
  float x = 1538.46*ratio;                //na podstawie charakterystyki z dokumentacji
  float ppm = pow(x, -1.709);

  /* -------------------------jezeli zostawy wywolane przerwania ------------------------------------ */
  if(tell_T == true){sayNumber((int)temperature_p);  tell_T = false;digitalWrite(8, LOW);} //jezeli zostalo wywolane przerwanie - sayNumber(), wyzeruj flage, zgas diode
  if(tell_H == true){sayNumber((int)humidity_p);  tell_H = false;digitalWrite(8, LOW);}
  if(tell_P == true){sayNumber((int)pressure_p);  tell_P = false;digitalWrite(8, LOW);}
  if(tell_C == true){sayNumber((int)ppm);  tell_C = false;digitalWrite(8, LOW);}
  
  /* -------------------------- wysylanie danych ---------------------------------------------------- */
  //Jeśli minela ponad minuta - wyslij dane
  if (aktualnyCzas-zapamietanyCzas_D >= 60000UL) {
    zapamietanyCzas_D = aktualnyCzas;//Zapamietaj aktualny czas
 //Skalowanie wartosci na string
  wilgotnosc_str = (String)humidity_p;
  updateDHT(temperatura_str, wilgotnosc_str); //wysylam dane
  }
  if (aktualnyCzas-zapamietanyCzas_B >= 61000UL) {
    zapamietanyCzas_B = aktualnyCzas;
  temperatura2_str = (String)temperature_p;
  cisnienie_str = (String)pressure_p;
      updateBMP(temperatura2_str, cisnienie_str);
  }
  if (aktualnyCzas-zapamietanyCzas_M >= 62000UL) {
    //Zapamietaj aktualny czas
    zapamietanyCzas_M = aktualnyCzas;
    ppm_str = (String)ppm;
      updateMQ(ppm_str);
  }
  //delay(100); //chyba usunac
}

void updateDHT(String a, String b){
  //Serial.println(a);//usunac
  //Serial.println(b);//usunac
  delay(200);
  GET = "GET /dht.php?temperature="+a+"&humidity="+b+" HTTP/1.1"; //tworzenie get requesta
  String command = GET;
 command += "\r\n";
 command += "Host: 192.168.43.85"; //ip komputera na ktorym jest serwer
 command += "\r\n\r\n";
  monitor.println("AT+CIPSTART=4,\"TCP\",\"192.168.43.85\",80"); //192.168.43.85 //rozpoczecie komunikacji z serwerem
  delay(500);//bylo 5000
  monitor.print("AT+CIPSEND=4,");//wysylanie 4 kanal, dlugosc wiadomosci
  monitor.println(command.length());  
  delay(300);
  monitor.print(command); //wyslanie komendy
  delay(300);//bylo 3000
  monitor.println("AT+CIPCLOSE=4");//zamykamy polaczenie
}
void updateBMP(String a, String b){
  GET = "GET /bmp.php?temperature_bmp="+a+"&humidity_bmp="+b+" HTTP/1.1";
  String command = GET;
 command += "\r\n";
 command += "Host: 192.168.43.85";
 command += "\r\n\r\n";
  monitor.println("AT+CIPSTART=4,\"TCP\",\"192.168.43.85\",80");
  delay(500);
  ...
  delay(300);
  monitor.println("AT+CIPCLOSE=4");
}
void updateMQ(String a){
  GET = "GET /mq.php?ppm="+a+" HTTP/1.1";
  String command = GET;
 command += "\r\n";
 command += "Host: 192.168.43.85";
 command += "\r\n\r\n";
...
void connect_c(){
  monitor.println("AT+CWMODE=1"); //tryb pracy esp - klient
  delay(2000);
        String cmd = "AT+CWJAP=\""+NazwaSieci+"\", \""+HasloSieci+"\""; //polaczenie z siecia
        delay(200);
  monitor.println(cmd);
  delay(2000);
}
